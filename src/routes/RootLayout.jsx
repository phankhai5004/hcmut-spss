...
function RoutingButton({ text, to, icon: Icon }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `group mt-3 flex h-10 w-full flex-col items-start justify-center gap-2 self-stretch rounded border-[1px] border-[#F5F2FA] hover:border-[#000F5D] ${isActive && 'border-[#000F5D]'}`
      }
    >
      {({ isActive }) => (
        <div
        ...
        >
          {Icon && (
            <Icon
              className={`h-[18px] w-[18px] fill-[#525A92] group-hover:fill-[#000F5D] ${isActive && 'fill-[#000F5D]'}`}
            />
          )}
          <p
            className={`text-[14px] font-normal text-[#525A92] group-hover:text-[#000F5D] ${isActive && 'text-[#000F5D]'}`}
          >
            {text}
          </p>
        </div>
...
export default function RootLayout() {
  return (
    <div className="flex h-lvh gap-5 p-3">
      ...
      <div className="h-full w-full rounded-xl bg-[#F5F2FA]">
        <Outlet />
      </div>
    </div>
  );
